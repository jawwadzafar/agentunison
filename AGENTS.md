# AGENTS.md — AgentUnison

Canonical instructions for coding agents working on AgentUnison itself (the CLI that
...
- Never write outside the repository root or under `.git/`; the tool's own invariants apply to
  its development too (see `dev_docs/2026-09-04/agentunison/005-migration-and-safety.md`).
...
- Design + review record: `dev_docs/2026-09-04/agentunison/001…008`, ADRs in `docs/adr/`.

## Commands
```bash
npm test                                  # all suites (node --test on .ts, Node ≥ 22.18)
npm run typecheck && npm run build        # tsc strict; emits dist/
AGENTUNISON_LIVE=1 npm run test:live     # probes installed harness binaries (opt-in)
...
- `node bin/agentunison.js verify` on this repository must pass (CI runs it).

<!-- agentunison:begin id=382f6cbf -->
