...
| CLI | `init inspect audit plan apply verify doctor uninstall`, `--json`, exit codes 0/1/2/4/7; `bin/agentconcord.js` over `dist/` |
...
| live (`AGENTCONCORD_LIVE=1 AGENTCONCORD_LIVE_API=1`) on macOS | **claude verified** (debug log: project skills loaded via `.claude/skills` link), **codex verified** (composed prompt contains the managed-block nonce and every skill), **opencode verified** (skills listed once each) — Claude Code 2.1.260, Codex 0.144.4, OpenCode 1.18.27 |
| build | `tsc -p tsconfig.build.json` → `dist/`; `bin/agentconcord.js --help` runs |
| dogfood | `agentconcord init --approve all` on this repository: AGENTS.md (authored) + managed block, CLAUDE.md shim, `.claude/skills → ../.agents/skills`, ledger; `node bin/agentconcord.js verify` OK; `verify --live`: codex + opencode verified, claude structural-only without `--allow-api-calls` |
| fresh clone | `git clone` of this repo: `.claude/skills` symlink and `.agents/skills/.gitkeep` survive; `tsc` build; `node bin/agentconcord.js verify` → structural OK; 34 tests pass in the clone |
...
- Publish to npm (`npx agentconcord`) and push to GitHub are pending the owner's go.
