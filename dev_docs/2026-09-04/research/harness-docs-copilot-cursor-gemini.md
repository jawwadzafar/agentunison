...
### E.2 Non-interactive "what got loaded" inspection (for AgentConcord real-harness verification)
...
- Implication for AgentConcord: generating **real files** (copies) per harness path is the only officially-supported strategy; symlink/junction-based fan-out is unsupported-or-undocumented on every harness and demonstrably broken in past versions of all three.
