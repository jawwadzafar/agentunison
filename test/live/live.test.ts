...
const LIVE = process.env['AGENTCONCORD_LIVE'] === '1';

test('live: installed harnesses see the canonical instructions and skills', { skip: LIVE ? false : 'set AGENTCONCORD_LIVE=1 to run against installed harness binaries' }, () => {
  const r = tmpRepo('ac-live-');
  fs.mkdirSync(path.join(r, '.agents/skills/probe-skill'), { recursive: true });
  fs.writeFileSync(path.join(r, '.agents/skills/probe-skill/SKILL.md'), '---\nname: probe-skill\ndescription: Use when probing agentconcord live verification.\n---\nProbe.\n');
  cmdInit(buildCtx(r), silentIO, { targets: ['claude', 'codex', 'opencode'], approve: [], yes: true, allowDelete: false });
  const results = verifyLive(buildCtx(r), { allowApiCalls: process.env['AGENTCONCORD_LIVE_API'] === '1' });
  for (const res of results) {
    ...
    if (res.harness === 'claude' && process.env['AGENTCONCORD_LIVE_API'] !== '1') { assert.equal(res.status, 'structural-only'); continue; }
